...
@Slf4j
@RestController
@Api(description = "达标清单管理接口")
@RequestMapping("/achievelist")
public class AchieveListController extends BaseController<AchieveList, String> {

    @Autowired
    private AchieveListService achieveListService;

    @Override
    public AchieveListService getService() {
        return achieveListService;
    }
    ...
    public Result<Object> deleteById(@PathVariable String id){
        ...
        long num = iWhiteListService.countBylistTypeAndlistId( LinkTypeConstant.ACHIEVE,id);
        if(num > 0){
    ...
    public Result<Object> batchDeleteByIds(@PathVariable String[] ids){
        ...
        for(String id:ids){
            ...
            long num = iWhiteListService.countBylistTypeAndlistId( LinkTypeConstant.ACHIEVE, id);
            if(num > 0){
                AchieveList achieveList =  getService().get(id);
                name.add(achieveList.getName());
            }else {
                getService().delete(id);
            }
        }
    ...
    public Result<Page<AchieveList>> listByCondition(@ModelAttribute AchieveList achieveList,
                                                            @ModelAttribute SearchVo searchVo,
                                                            @ModelAttribute PageVo pageVo){
        String appid = UserContext.getAppid();
        achieveList.setAppid(appid);
        Page<AchieveList> page = achieveListService.findByCondition(achieveList, searchVo, PageUtil.initPage(pageVo));
        return new ResultUtil<Page<AchieveList>>().setData(page);
    }
    ...
    public Result<AchieveList> update(@ModelAttribute AchieveList entity){
        AchieveList e =null;
        long total = iAchieveListRecordService.countByListId(entity.getId());
        String appid = UserContext.getAppid();
        List<AchieveList> lists= achieveListService.findByAppidAndName(appid,entity.getName());
        if(lists.size() > 0){
            if(lists.size()>1){
                return new ResultUtil<AchieveList>().setErrorMsg("受限名单名称已被占用");
            }
            AchieveList achieveList = lists.get(0);
            if(!StringUtils.equals(achieveList.getId(),entity.getId())){
                return new ResultUtil<AchieveList>().setErrorMsg("受限名单名称已被占用");
            }
        }
        AchieveList old =  achieveListService.get(entity.getId());
        if(old!=null){
        ...
        if(total == 0){
            ...
            if(comparaFlag == -1){
                return new ResultUtil<AchieveList>().setErrorMsg("过期时间不得早于今天");
            }
            ...
            getService().update(e);
            // 更新过期时间
            achieveListRecordService.updateCacheTime(e);
        }
        achieveListRecordService.loadCache(entity.getId());
        return new ResultUtil<AchieveList>().setData(e);
    }
